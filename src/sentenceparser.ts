...
export function makeToken(t : IFErBase.IWord, index : number, T : any ) {
  ...
  if (t.rule.wordType === IFModel.WORDTYPE.CATEGORY) {
    return { image : "CAT",  startOffset : index, tokenType : T["ACategory"].tokenType };
  };
  if (t.rule.wordType === 'F') {
    //console.log(JSON.stringify(t));
    return { image : "FACT",  startOffset : index, tokenType : T["AFact"].tokenType };
  }
  if (t.rule.wordType === 'D') {
    //console.log(JSON.stringify(t));
    return { image : "DOM",  startOffset : index, tokenType : T["ADomain"].tokenType };
  }
  if (t.rule.wordType === 'A') {
    //console.log(JSON.stringify(t));
    return { image : "ANY",  startOffset : index, tokenType : T["AnANY"].tokenType };
  }
